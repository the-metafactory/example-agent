 ...
 *   1. EXAMPLE_AGENT_ENV_FILE (explicit path)
 *   2. ~/.config/metafactory/example-agent/.env   — the PRINCIPAL-owned overlay
...
/** The principal-owned config dir: ~/.config/metafactory/example-agent/. */
export function principalConfigDir(): string {
  return join(homedir(), ".config", "metafactory", "example-agent");
}
...
function resolveEnvFilePath(): string | null {
  const explicit = process.env.EXAMPLE_AGENT_ENV_FILE;
...
export function loadBrainEnv(): string | null {
  ...
  if (path === null) {
    process.stderr.write("example-agent: no env file found — relying on injected env / defaults\n");
    return null;
  }
  let filled = 0;
  try {
    ...
    process.stderr.write(
      `example-agent: failed to read env file ${path}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return null;
  }
  process.stderr.write(`example-agent: loaded ${filled} var(s) from ${path}\n`);
  return path;
}
