 ...
 *
 *   displayName:
 *     1. env  EXAMPLE_AGENT_DISPLAY_NAME   (cortex stack injects, or the overlay .env)
 *     2. DEFAULT_DISPLAY_NAME (below)
 *
 *   persona file:
 *     1. env  EXAMPLE_AGENT_PERSONA        (explicit path)
 *     2. ~/.config/metafactory/example-agent/persona.md   (principal overlay)
...
export const DEFAULT_DISPLAY_NAME = "Example Agent";
...
function resolvePersona(): { path: string; source: AgentIdentity["personaSource"] } {
  const fromEnv = process.env.EXAMPLE_AGENT_PERSONA;
...
export function resolveIdentity(): AgentIdentity {
  const nameFromEnv = process.env.EXAMPLE_AGENT_DISPLAY_NAME;
  const displayName =
