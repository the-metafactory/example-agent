/**
 * Example Agent brain — daemon socket shell (cortex-brain/v1).
 *
...
import { ExampleBrain, encodeEffectLine, type BrainEffect } from "./handler";
...
if (socketPath === undefined || socketPath.length === 0 || token === undefined) {
  process.stderr.write(
    "example-agent: CORTEX_BRAIN_SOCKET / CORTEX_BRAIN_SOCKET_TOKEN missing — " +
...
process.stderr.write(
  `example-agent: identity="${identity.displayName}" persona=${identity.personaSource} (${identity.personaPath})\n`,
);
...
const brain = new ExampleBrain({ send, identity: { displayName: identity.displayName } });
...
const socket = await Bun.connect({
  ...
  socket: {
    ...
    data(_s, chunk) {
      for (const line of decoder.push(chunk)) {
        ...
        if (event === null) {
          ...
          process.stderr.write("example-agent: dropping unrecognized event line\n");
          continue;
        }
    ...
    close() {
      ...
      process.stderr.write("example-agent: socket closed by host — exiting\n");
      process.exit(shuttingDown ? 0 : 1);
    },
    error(_s, err) {
      process.stderr.write(`example-agent: socket error: ${err.message}\n`);
      process.exit(1);
    },
  },
});
...
sockRef = socket;
flushOut();
process.stderr.write("example-agent: connected\n");
